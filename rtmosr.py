"""
RTMoSR architecture — Real-Time Multi-Output Super Resolution
From: https://github.com/rewaifu/RTMoSR
License: MIT
"""
...
class Conv3XC(nn.Module):
    def __init__(self, c_in, c_out, gain=2, s=1, bias=True):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(
                c_in * gain, c_out * gain, kernel_size=3, stride=s, padding=0, bias=bias
            ),
            nn.Conv2d(c_out * gain, c_out, kernel_size=1, padding=0, bias=bias),
        )
        self.eval_conv = nn.Conv2d(
            c_in, c_out, kernel_size=3, padding=1, stride=s, bias=bias
        )
...
class OmniShift(nn.Module):
    def __init__(self, dim=48):
        ...
        self.conv5x5 = nn.Conv2d(
            dim, dim, kernel_size=5, padding=2, groups=dim, bias=True
        )
    ...
    def reparam_5x5(self):
        ...
        self.conv5x5_reparam.weight = nn.Parameter(combined_weight.to(device))
        self.conv5x5_reparam.bias = nn.Parameter(combined_bias.to(device))
...
class GatedCNNBlock(nn.Module):
    def __init__(
        self, dim=64, expansion_ratio=8 / 3, conv_ratio=1.0, dccm=True, se=False
    ):
        super().__init__()
