...
def make_frame(n, f):
    """Process a single frame through RTX VSR — all on GPU, no IPC."""
    ...
    # Upload to GPU and run RTX VSR (locked for thread safety)
    t0 = time.time()
    with _gpu_lock:
        tensor = (
    ...
    if _frame_count % 50 == 0:
        ...
        print(
            f"RTX VSR: {_frame_count} frames, {fps:.1f} fps | read={t_read * 1000:.1f}ms gpu={t_gpu * 1000:.1f}ms write={t_write * 1000:.1f}ms",
            file=sys.stderr,
        )
